...
function renderContact(contact) {
  ...
  // create the content inserting the object info
  const content = `
...
function render(contacts) {
  ...
  // get the contact HTML
  const content = contacts.map(renderContact).join("");
    
  contactSection.innerHTML = content; 
  // call the loadCities() function to ensure that the list of cities in the filter is updated
  loadCities(contacts); 
}; 
...
function filterByCity(city) {
  // filter the array of contacts by the given city 
...
function filterHandler() {
  ...
  // attach change event listener to filterOptions select
  filterOptions.addEventListener("change", (event) => {
    event.preventDefault(); 
    // get the value selected by the user 
    const value = event.target.value; 
   // if the value is 0, call render(contacts)
    if (value === "0") {
      render(contacts)
   // else call filterByCity() w/ the user-selected city
    } else {
...
function loadCities(contacts) {
  // filter contacts array to grab the cities
  const cities = contacts.map((contact) => contact.address.city); 
  // select the select with ID filterOptions 
  const select = document.querySelector("#filterOptions");
  // for each city, create an option element with text and value
  cities.forEach((city) => {
...
// ^^ remove innerHTML of select element? or just add innerHTML to the select element instead of what I have above? 

/*
  Remove the contact from the contact list with the given id.
*/
function deleteContact(id) {
  // remove the contact with the given id from the contact list
  // find the id from the contact list
  contacts.find((contact) => {
    // if the id matches the contact id
    if (contact.id === id) {
      // remove the contact from the contact list
      
     // if the id doesn't exist, no contacts are deleted
    } 
  });
  
  // remove the contact removeChild? 
}; 
